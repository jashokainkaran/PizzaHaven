...
if (isset($_GET['delete_id'])) {
    ...
    echo "<script>
        ...
        if (confirmDelete) {
            window.location.href = 'drinks_dashboard.php?confirm_delete_id=$itemId';
        } else {
            window.location.href = 'drinks_dashboard.php';
        }
    </script>";
    exit();
}
...
if (isset($_GET['confirm_delete_id'])) {
    ...
    // Redirect back to the drinks dashboard
    header("Location: drinks_dashboard.php");
    exit();
}
?>
...
<div class="container mx-auto px-4 mt-3">
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-2xl font-bold text-secondary">Beverages Table</h2>
        
        <a href="../admin/add_forms/add_items_form.php" class="bg-primary text-white px-6 py-2 mt-4 rounded-lg font-medium hover:bg-red-700">Add Beverage</a>
    </div>
...
$sqlDrinks = "SELECT * FROM menu WHERE type = 'drink'";
...
if ($resultDrinks->num_rows > 0) {
    while ($drinkRow = $resultDrinks->fetch_assoc()) {
        ...
        $drinkCategory = $drinkRow['category'];

        echo "<tr class='bg-white border-b'>";
        echo "<th scope='row' class='px-4 sm:px-6 py-4 font-bold whitespace-nowrap'>$drinkId</th>";
        echo "<td class='px-4 sm:px-6 py-4'><img src='$drinkImage' alt='$drinkName' class='w-20 h-20 object-cover rounded'></td>";
        echo "<td class='px-4 sm:px-6 py-4'>$drinkName</td>";
        echo "<td class='px-4 sm:px-6 py-4'>$drinkDescription</td>";
        echo "<td class='px-4 sm:px-6 py-4'>$drinkCategory</td>";
        echo "<td class='px-4 sm:px-6 py-4'>Rs $drinkPrice</td>";
        echo "<td class='px-4 sm:px-6 py-4'>
                <a href='./edit_forms/edit_items_form.php?id=$drinkId' class='font-medium text-blue-600 hover:underline'>Edit</a>
                <a href='pizza_dashboard.php?delete_id=$drinkId' class='font-medium text-red-600 hover:underline ms-3'>Delete</a>
            </td>";
        echo "</tr>";
    }
} else {
    echo "<tr><td colspan='7' class='text-center py-4 text-gray-500'>No Drinks Found</td></tr>";
}
?>
            </tbody>
        </table>
